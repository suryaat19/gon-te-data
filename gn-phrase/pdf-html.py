...
PDF_PATH = "Gondi Phrase Book.pdf"
API_KEY = "sk_1i67upfa_23OG9u8MbGfmMNnU3iZxCozC"
PAGES_PER_CHUNK = 10
MAX_PAGES = 128
HTML_STORAGE_DIR = "saved_html_parallel_2"
...
def extract_pdf_to_html_with_sdk():
    ...
    for start_page in range(81, total_pages, PAGES_PER_CHUNK):
        ...
        if os.path.exists(final_html_name):
            print(f"Skipping pages {start_page+1}-{end_page}, already processed.")
            continue
        ...
        for attempt in range(max_retries):
            try:
                ...
                with zipfile.ZipFile(chunk_zip_filename, 'r') as zip_ref:
                    ...
                    if html_files:
                        extracted_path = zip_ref.extract(html_files[0], HTML_STORAGE_DIR)
                        if os.path.exists(final_html_name):
                            os.remove(final_html_name)
                        os.rename(extracted_path, final_html_name)
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"Failed to process pages {start_page+1}-{end_page} after 3 attempts.")
